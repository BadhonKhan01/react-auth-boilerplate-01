let paginated = (model, populate) => {
    return async (req, res, next) => {
        ...
        |--- @Get Mongodb Query
        ------------------------------------------------------*/
        results.items = await model.find({ deleteItem: 0 })
                                    .limit(parPage)
                                    .skip(startIndex)
                                    .sort([['_id','desc' ]])
                                    .populate(populate)
                                    .exec();
        results.countItems = await model.find({ deleteItem: 0 }).countDocuments();
        results.countTrashItems = await model.find({ deleteItem: 1 }).countDocuments();
        const totalItems = await model.find({ deleteItem: 0 }).exec();
